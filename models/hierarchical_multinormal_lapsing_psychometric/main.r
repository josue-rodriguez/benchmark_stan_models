...
grand_mean <- mean(fit_samples$mum)

# condition
cond <- as.data.frame(fit_samples$factor_alpha)
cond_mean <- apply(cond, 2, mean)

# subject
subj <- as.data.frame(fit_samples$subject_alpha)
subj_mean <- apply(subj, 2, mean)

# subject by condition
subj_cond <- as.data.frame(fit_samples$interaction_alpha) # looks like these are the 14 conditions * the 17 subjects.
subj_cond_mean <- apply(subj_cond, 2, mean)

# width
width_mean <- mean(fit_samples$muw)

# width by condition
subj_width <- as.data.frame(fit_samples$subject_beta)
...
df <- df |>
  mutate(
    overall_midpoint = (grand_mean + cond_mean + subj_cond_mean),
...
for (s in 1:n_plot_samps) {
  midpoint <- with(fit_samples, mum[plot_samps[s]])
  width <- with(fit_samples, muw[plot_samps[s]] + subject_beta[plot_samps[s]])
  psi[s, ] <- chance_performance + (1 - chance_performance) *
    logistic((xx - midpoint) / width)
  for (cn in 1:nCond) {
    midpoint_cn <- midpoint + fit_samples$factor_alpha[plot_samps[s], cn]
    psi_cond[s, cn, ] <- chance_performance + (1 - chance_performance) *
      logistic((xx - midpoint_cn) / width)
  }
  for (sn in 1:nSubj) {
    midpoint_sn <- midpoint + fit_samples$subject_alpha[plot_samps[s], sn]
    lapse_sn <- fit_samples$lapse[plot_samps[s], sn]
...
#################################
matplot(xx * global_window_rescale, t(psi),
  type = "l", lty = 1,
...
abline(h = .5, lty = 2)
for (cn in 1:nCond) {
  matlines(xx * global_window_rescale, t(psi_cond[, cn, ]),
    lty = 1,
    col = rainbow(nCond, alpha = .1)[cn]
  )
}
legend("bottomright",
  legend = c("Group", levels(dat$condition)),
...
abline(h = .5, lty = 2)
for (sn in plot_subj) {
  matlines(xx * global_window_rescale, t(psi_subj[, sn, ]),
    lty = 1,
